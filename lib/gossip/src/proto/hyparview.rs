...
use super::{IO, PeerData, PeerInfo, PeerPrefixEntity, util::IndexSet};
...
pub enum Message<PI> {
    ...
    /// other peers.
    Shuffle(Shuffle<PI>),
    /// Peers reply to [`Message::Shuffle`] requests with random peers from their active and
    /// passive views.
...
impl Default for Config {
    /// Default values for the HyParView layer (from the paper, p9)
    fn default() -> Self {
        Self {
    ...
    RG: Rng,
{
    ...
    fn send_disconnect(&mut self, peer: PI, alive: bool, io: &mut impl IO<PI>) {
        ...
        // prevent the other node from running out of connections.
    ...
    fn on_join(&mut self, peer: PI, data: Option<PeerData>, io: &mut impl IO<PI>) {
        ...
        // Forward the join to all other nodes in our active view
    ...
    fn on_forward_join(&mut self, sender: PI, message: ForwardJoin<PI>, io: &mut impl IO<PI>) {
        let peer_prefix = message.peer.id;
        // If the peer is already in our active view, the TTL expired, or we only have 1 active
        // peer, (re-)add to active view and renew our neighbor relationship.
        if self.active_view.contains(&peer_prefix)
            || message.ttl.expired()
            || self.active_view.len() <= 1
        {
            self.insert_peer_info(message.peer, io);
            self.send_neighbor(peer_prefix, Priority::High, io);
        } else {
            // At PRWL, insert into passive view
            if message.ttl == self.config.passive_random_walk_length {
                self.add_passive(peer_prefix, message.peer.data.clone(), io);
            }
            // Forward to a random active peer (not the sender)
            if !self.active_view.contains(&peer_prefix)
                && !self.pending_neighbor_requests.contains(&peer_prefix)
                && let Some(next) = self
                    .active_view
    ...
    fn send_shuffle_reply(&mut self, to: PI, len: usize, io: &mut impl IO<PI>) {
        ...
        // If we don't have enough passive nodes, fill with active nodes.
        if nodes.len() < len {
            nodes.extend(
                self.active_view
    ...
    fn remove_active(&mut self, peer: &PI, reason: RemovalReason, io: &mut impl IO<PI>) {
        if let Some(idx) = self.active_view.get_index_of(peer) {
            let removed_peer = self.remove_active_by_index(idx, reason, io);
            if let Some(removed) = removed_peer {
                self.refill_active_from_passive(&[&removed], io);
            }
        }
    }
