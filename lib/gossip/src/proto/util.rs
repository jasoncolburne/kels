...
/// hash values.
///
/// This is wrapper around [indexmap::IndexSet] with utility methods
        ...
        for (time, key) in drain {
            match self.map.entry(key) {
                ...
                hash_map::Entry::Occupied(_entry) => {
                    // Entry was re-added with a later time; do not remove.
                }
                hash_map::Entry::Vacant(_) => {
                    // Entry was already removed (inserted multiple times).
                }
            }
        }
        count
...
mod test {
    ...
    fn hex() {
        ...
        assert_eq!(
            id,
            Id::from_str(&format!("{id}")).unwrap_or_else(|_| Id::from_bytes([0; 32]))
        );
        assert_eq!(
            &format!("{id}"),
