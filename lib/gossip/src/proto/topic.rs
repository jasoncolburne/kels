//! Per-topic gossip protocol state
//!
...
use super::{
    DEFAULT_MAX_MESSAGE_SIZE, MIN_MAX_MESSAGE_SIZE, PeerData, PeerPrefixEntity,
...
pub struct Config {
    ...
    /// large messages.
    ///
    /// At minimum, this size should be large enough to send gossip control messages. This can
    /// vary, depending on the size of the [`PeerPrefixentity`] you use and the size of the
    /// [`PeerData`] you transmit in your messages.
    ///
...
impl<PI: PeerPrefixEntity> State<PI, StdRng> {
    ...
    pub fn new(me: PI, me_data: Option<PeerData>, config: Config) -> Self {
        Self::with_rng(me, me_data, config, StdRng::from_os_rng())
    }
}
...
impl<PI: PeerPrefixEntity, R: Rng> State<PI, R> {
