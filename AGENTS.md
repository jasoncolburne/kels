...
- **Privileged-divergence-is-terminal; universal locking.** A privileged event (KEL: `Rot`/`Ror`/`Dec`; SEL: `Est`/`Sea`/`Dec`; IEL: `Evl`/`Dec`) whose acceptance would create or join a divergent set is rejected at the merge layer per §Privileged Divergence is Terminal — the seal never forks, and divergent sets contain only non-privileged events by construction. Once any privileged event has landed at `v_d` on a chain, the seal-cap (`parent_serial >= seal_serial`) rejects every subsequent submission whose parent sits in the locked portion — no carve-outs, no boundary cases. Federation races between concurrent privileged submissions do not structurally converge at the protocol layer; convergence is provided at the federation layer via the irreconcilable-prefix table (see [#205](https://github.com/jasoncolburne/kels/issues/205)). See [docs/design/protocol-doctrine.md §Privileged Divergence is Terminal](docs/design/protocol-doctrine.md#privileged-divergence-is-terminal) and [§Limit of the doctrine — concurrent privileged event races](docs/design/protocol-doctrine.md#concurrent-privileged-event-races).
...
- **Cascade-reincept honesty.** Reincept is needed when the *primitive itself* is disputed at the federation layer (surfaced via the irreconcilable-prefix table per [#205](https://github.com/jasoncolburne/kels/issues/205)), not when a referenced primitive is:
  - **IEL disputed** → every SEL bound to it must reincept under a new prefix.
  - **SEL disputed** → the SEL is dead in place; nothing downstream cascades.
  - **KEL disputed** → dependents only reincept when the disputed KEL actually anchored events on them AND the relevant policy lacks threshold redundancy. Policies with `M > N` across distinct custodians absorb single-member disputes via `Evl` rotating the disputed KEL out.

  The expensive case is federation-layer dispute on an **IEL at the root of a dependency tree** — partition identity hierarchies so any single dispute has bounded blast radius. See [docs/design/protocol-doctrine.md §Adversary Patience and Policy Redundancy](docs/design/protocol-doctrine.md#adversary-patience-and-policy-redundancy).

- **Federation convergence.** Gossip propagation + deterministic effective-SAID resolution ensures every chain converges on the same semantic state across all nodes where the protocol layer can converge. Cross-node priv-vs-priv races and other federation-layer disputes are surfaced via the irreconcilable-prefix table at the federation layer (per [#205](https://github.com/jasoncolburne/kels/issues/205)). End-verifiability over data-from-any-source depends on this two-layer convergence. Single-node deployments forfeit this property. See [docs/design/protocol-doctrine.md §Federation Convergence](docs/design/protocol-doctrine.md#federation-convergence).
...
**Key Event Log (KEL)** — append-only chain of key events sharing a prefix. Each event links to the previous via SAID. Forward commitments via `rotationHash = Blake3(next_public_key)`. `Rec`/`Ror`/`Dec` require dual signatures. Delegation trust is NOT verified by the service. See `docs/design/primitives/data/event-logs/kel/events.md`, `docs/design/primitives/data/event-logs/kel/verification.md`, `docs/design/protocol-doctrine.md` §Part 3 (cross-primitive verification doctrine: streaming, tokens, effective-SAID synthetic comparison).

**Divergence** — conflicting events at the same serial. Chain transitions to Divergent (recoverable via `Rec`/`Rpr`). Privileged events whose acceptance would create or join a divergent set are rejected at the merge layer per privileged-divergence-is-terminal — divergent sets contain only non-privileged events by construction. Cross-node priv-vs-priv races surface at the federation layer via the irreconcilable-prefix table (per [#205](https://github.com/jasoncolburne/kels/issues/205)). See `docs/design/primitives/data/event-logs/kel/event-log.md`, `docs/design/primitives/data/event-logs/kel/recovery-workflow.md`, `docs/design/primitives/data/event-logs/kel/reconciliation.md`.

**Effective SAID** — tip SAID for normal chains (including decommissioned); `hash_effective_said("divergent:{prefix}")` for divergent; `hash_effective_said("irreconcilable:{prefix}")` for irreconcilable. Under #214 the irreconcilable-prefix surface is federation-layer-sourced (per #205), not per-node-state-derived — the service returns the synthetic in the response when it knows the prefix is irreconcilable. See `docs/design/primitives/data/event-logs/kel/merge.md`.
...
**Identity Event Log (IEL)** — chain primitive that governs an identity. Carries `authPolicy` and `governancePolicy` declarations (`Icp`) and evolutions (`Evl`); terminal via `Dec` (decommission). Every IEL event is governance-authorized (anchored under the chain's `governancePolicy`); `authPolicy` is the per-event policy declaration consumed by SEL `Est`/`Upd` via `ielEvent` binding. Every introduced/evolved policy must be `immune: true`. IEL has no per-node Divergent state — every IEL event is privileged, so any second event at the same serial is rejected at the merge layer per privileged-divergence-is-terminal. Cross-node priv-vs-priv races surface at the federation layer via the irreconcilable-prefix table (per [#205](https://github.com/jasoncolburne/kels/issues/205)). IEL has no `Rpr`.
