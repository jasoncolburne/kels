...
- **Privileged-divergence-is-terminal; order-independent.** A non-archiving privileged event (KEL: `Ror`/`Dec`; SEL: `Sea`/`Dec`; IEL: `Evl`/`Sea`/`Dec`) landing in a divergent set transitions the chain to Contested regardless of arrival order — including a competing extension at `Dec`'s serial that transitions a Decommissioned chain to Contested. Construction: `previous = v_{d-1}.said`, `serial = d`, lands at `v_d`. The locked-portion bound (`previous` must not be in the chain's locked portion) is enforced on KEL/SEL by the seal-cap; on IEL there is no separate bound since every IEL event is privileged. See [docs/design/protocol-doctrine.md §Privileged Divergence is Terminal](docs/design/protocol-doctrine.md#privileged-divergence-is-terminal) and [§Order-independent divergent transitions](docs/design/protocol-doctrine.md#order-independent-divergent-transitions).
