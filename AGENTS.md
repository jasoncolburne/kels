...
- **Cascade-reincept honesty.** A contested *chain* (IEL or SEL) invalidates everything bound to it — dependents reincept under a new prefix. A contested *KEL* referenced by some policy only invalidates dependents when the contested KEL actually anchored events AND the policy lacks threshold redundancy; threshold-redundant policies absorb single-member contest via `Evl` rotating the contested KEL out. The expensive case is a contested chain at the root of a dependency tree — don't put your entire dependent tree under a single root chain. See [docs/design/protocol-doctrine.md §Adversary Patience and Policy Redundancy](docs/design/protocol-doctrine.md#adversary-patience-and-policy-redundancy).
