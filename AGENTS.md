...
- On an **already-divergent chain**, `Cnt.previous = v_{d-1}.said`. Freeze-on-divergence keeps the shorter branch single-event with its tip at `v_d`, so its `v_{tip-1}` is `v_{d-1}` — the divergence ancestor (the unique shared parent of all events at `v_d` by the divergence invariant; both linear and divergent shapes are the same `v_{tip-1}` rule applied differently). Cnt joins the divergent set at `v_d`. Cnt authorizes through the same policy/key state required to accept the events that already extend `v_{d-1}` — i.e., the commitments made at `v_{d-1}`.
