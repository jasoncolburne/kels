...
- **`Cnt` is the protocol-level termination event; its parent is the divergence ancestor.** `Cnt.previous = v_{d-1}.said`; `Cnt.serial = d`. Cnt lands at `v_d` extending the divergence ancestor, subject to the locked-portion bound — `Cnt.previous` must not be in the chain's locked portion. The bound is enforced on KEL/SEL by the seal-cap; IEL has no `Cnt` (every IEL event is privileged, so divergence is contested-terminal directly). On KEL specifically, the dual-signature against `v_{d-1}`'s commitments preserves operator recourse against signing-key-only `Rot` takeover. See [docs/design/protocol-doctrine.md §Privileged Divergence is Terminal §Repair-event conditions](docs/design/protocol-doctrine.md#privileged-divergence-is-terminal).
