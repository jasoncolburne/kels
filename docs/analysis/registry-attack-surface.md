...
- **Recovery:** Once detected, the operator submits a `rec` event (requires rotation + recovery key). The merge protocol truncates the attacker's events from the active chain (archiving them for audit) and resumes from the pre-compromise state under a new key. If the attacker has also compromised the rotation key, recovery still works as long as they don't hold the recovery key. If both rotation and recovery keys are compromised, contested-terminal is the structurally honest outcome — whichever party (operator or adversary) lands a privileged event (`Rot`/`Ror`/`Dec`) extending `v_{d-1}` (the last shared parent before the adversary's events) first fires privileged-divergence-is-terminal per [../design/protocol-doctrine.md §Privileged Divergence is Terminal](../design/protocol-doctrine.md#privileged-divergence-is-terminal), permanently freezing the KEL. The protocol has no dedicated `Cnt`/contest event; contestation is reached as a chain-state outcome of privileged-divergence rather than a discrete submission.
