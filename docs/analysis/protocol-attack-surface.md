...
- **Outcome:** Indistinguishable from legitimate operator-initiated termination. Reduces to current-state compromise (covered under [../design/security-invariant.md §Limit of the Doctrine](../design/security-invariant.md#limit-of-the-doctrine)).
...
- **Detection:** The verifier walk identifies the contested state; consumer trust degrades per the whole-chain-suspect rule. See [../design/security-invariant.md §Trust Model on Contested Chains](../design/security-invariant.md#trust-model-on-contested-chains).
...
- **Doctrine reference:** [../design/security-invariant.md §Compromise is Permanent](../design/security-invariant.md#compromise-is-permanent).
...
- **Mitigation:** `Dec` is recovery-revealing → privileged → advances the seal to its own version. The seal-cap rule (`event_version >= seal_version`) prevents any event from landing at a version strictly before the seal. A past-key-compromise adversary cannot create a fork at-or-before `Dec`; the verifier rejects with "Cannot land at version V — sealed by evaluation/recovery at version S". The decommissioned-state gate also rejects all submissions before the seal-cap fires. Combined: no event lands on a decommissioned KEL.
...
The IEL primitive governs identity authorization via `auth_policy` (consumed by SELs) and `governance_policy` (the chain's own gate). All IEL events — `Icp`, `Evl`, `Cnt`, `Dec` — are governance-authorized; this has structural consequences for divergence handling. See [../design/iel/event-log.md](../design/iel/event-log.md).
...
SELs are identity-rooted — every SEL binds at inception to an IEL prefix and resolves per-event authorization through specific IEL event SAIDs (`identity_event` field). See [../design/sel/event-log.md](../design/sel/event-log.md).
...
- **Doctrine reference:** [../design/security-invariant.md §Forks are Seal-Bounded](../design/security-invariant.md#forks-are-seal-bounded) (per-event parent-monotonic on SEL).
...
- **Doctrine reference:** [../../AGENTS.md §System Thesis](../../AGENTS.md#system-thesis) (cascade-reincept honesty) + [../design/security-invariant.md §Limit of the Doctrine](../design/security-invariant.md#limit-of-the-doctrine) (cascade-reincept bullet).
