...
**Signing-key-only `rot` takeover (operator recourse):** A nuance worth calling out. If an adversary captures only the signing key (recovery key remains in separate custody) and submits a `rot` at `v_N`, the operator's `Rec` with `previous = v_{N-1}.said` (divergence-ancestor-extending shape) requires keys committed by `v_{N-1}`: the `v_N`-current signing key (revealed by the adversary's `rot` — both parties have it) AND the `v_N`-current recovery key (NOT revealed by `rot` — only the operator has it, since recovery is revealed only by `rec`/`ror`/`dec`). Operator's dual-signature succeeds; adversary's does not. `Rec` lands at `v_N` and the discriminator archives the adversary's branch; the chain recovers, linear, tip = `Rec` at `v_N`. See [../design/primitives/data/event-logs/kel/event-log.md §Operator recourse against signing-tier Rot takeover](../design/primitives/data/event-logs/kel/event-log.md#operator-recourse-against-signing-tier-rot-takeover).
...
- **Mitigation:** Privileged-divergence-is-terminal fires when a non-archiving privileged event (`Ror`/`Dec`) joins a divergent set. Two shapes:
  - **Sequential: one party's `rec` lands cleanly, the other submits a non-archiving privileged event.** Loser's `Ror`/`Dec` extends `v_{d-1}.said` on the post-`rec` linear chain (where `v_d` is the post-`rec` tip), lands at `v_d`, creates a 2-event divergent set with a non-archiving privileged event in it; privileged-divergence fires; chain becomes contested-terminal. Effective SAID converges to `hash_effective_said("contested:{prefix}")` across nodes.
  - **Concurrent recovery-revealing submissions on different nodes.** Each `rec` lands via linear-chain rules on its submitting node (the local divergent set is archived); each node ends up linear with its own `rec` as tip at `v_d`. Gossip then delivers each `rec` to the other node; the locked-portion bound rejects the gossip-arriving `rec` (its `previous` sits in the now-locked portion behind the resident `rec`). Nodes do not structurally converge in this case; federation-level convergence is handled at the infrastructure layer via a contested-prefix table (see [../design/protocol-doctrine.md §Limit of the doctrine — concurrent-Rec/Rpr races](../design/protocol-doctrine.md#privileged-divergence-is-terminal) and [#205](https://github.com/jasoncolburne/kels/issues/205)).
- The second party retains the ability to force termination via `Ror`/`Dec` after a recovery has landed — the chain mathematics never let one party "keep the chain" against another holder of recovery authority. This is the correct security outcome.
...
- **Governance-evolution-window stale-bound fork-contest.** A stale-governance holder can land a non-archiving privileged event (`Sea`/`Dec`) on a fresh singleton branch off `v_{d-1}` of a dependent SEL if the live branch hasn't been ratcheted past their old IEL event via a Sea ratchet; pre-Icp camping on a permissionless SEL prefix before the legitimate operator's enrollment. Mitigated by the governance-evolution Sea ratchet (prompt `Sea` post-`Evl`) and structured enrollment flow.
