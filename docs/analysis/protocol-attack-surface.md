...
- **Terminal-contestation path via `Rot` race:** A rotation-tier adversary can also force contested-terminal by racing `Rot_adv` against a concurrent honest `Rot_op`. Both events extend `v_{d-1}` and both are privileged; the resulting 2-event privileged divergent set fires privileged-divergence-is-terminal. The chain becomes contested-terminal; operator recourse is reincept under a new prefix, not `Rec`. The bar to terminally damage the KEL via this path is rotation-tier compromise — strictly easier than the full tier-3 compromise required to forge `Ror`/`Dec`. Events at-or-below the chain's `lastSealAdvancingEvent` retain structural verifiability per [../design/protocol-doctrine.md §Pre-divergence verifiability survives contestation](../design/protocol-doctrine.md#pre-divergence-verifiability-survives-contestation), bounding the damage to forward extension. Tier-1 `Ixn`s the adversary may have planted between the last seal and the Rot race are NOT structurally trustworthy — they sit above the seal and are not consumer-durable until a (now-impossible) future seal would advance past them.
- **Operator path between the two outcomes:** the operator's choice of response determines which path the chain takes. `Rec` resolves cleanly; `Rot_op` racing the adversary's `Rot_adv` is contested-terminal. Operator-side monitoring catches adversary `Rot` before honest concurrent rotation, preventing the race; operational serialization on `Rot` issuance closes the race window. The contested-prefix table at the infrastructure layer ([#205](https://github.com/jasoncolburne/kels/issues/205)) surfaces the contested state for out-of-band convergence without recovering the chain.
- **Detection:** Divergence detection as above; the verifier surfaces `is_contested = true` if the divergent set contains any privileged event (`Rot`, `Ror`, or `Dec`).

The "Rotation key compromised" subsection above covers the tier-2 adversary in full. The cross-doc walkthrough of the clean Rec recovery path lives at [../design/primitives/data/event-logs/kel/event-log.md §Recourse against signing-key-only Rot takeover](../design/primitives/data/event-logs/kel/event-log.md#recourse-against-signing-key-only-rot-takeover). The Rot-race terminal path is the doctrinal threat-model shift; see [../design/protocol-doctrine.md §Tier-2 adversary terminal-contestation path](../design/protocol-doctrine.md#tier-2-adversary-terminal-contestation-path).
...
**Attack:** Submit events to a Contested (terminal) KEL.
...
### Submission to Already-Contested IEL

**Attack:** Submit any event to an IEL whose contested-state gate has already closed (any divergence on IEL is contested-terminal by privileged-divergence rule).
- **Mitigation:** Contested-state gate rejects all subsequent submissions, including events arriving via gossip at `v_d`.
- **Structural note:** IEL has no upgrade-rule path — every IEL event is privileged → no non-privileged divergent set can form on IEL → contested-terminal at first observation of any 2-event divergence. The 3rd-event-via-upgrade pattern that applies on KEL/SEL (non-privileged divergent set transitions to contested when a privileged event joins) is structurally vacuous on IEL.
...
- **Cascade effect:** SELs whose binding sits at-or-below the IEL's `lastSealAdvancingEvent` remain trust-evaluable — their `ielEvent` references the structurally-final at-or-below-seal segment. The seal-bound covers the divergent case by construction (on a contested IEL the seal stays at the prior linear-portion advance, strictly below `first_divergent_serial`). SELs that would forward-extend their binding against an above-seal `ielEvent` face the freeze (above-seal IEL events are either tier-1-only-auth in the seal-to-divergence gap or submitter-indistinguishable at-or-above divergence; the protocol provides no policy state for those bindings to resolve against). See [../design/primitives/data/event-logs/iel/event-log.md §Effect on Bound SELs](../design/primitives/data/event-logs/iel/event-log.md#effect-on-bound-sels).
