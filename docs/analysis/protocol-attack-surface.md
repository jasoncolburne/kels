...
- **Cascade effect:** SELs whose binding sits at-or-below the IEL's `lastSealAdvancingEvent` AND below `first_divergent_serial` remain trust-evaluable — their `ielEvent` references the structurally-final at-or-below-seal segment. SELs that would forward-extend their binding against an above-seal `ielEvent` face the freeze (above-seal IEL events are either tier-1-only-auth in the seal-to-divergence gap or submitter-indistinguishable at-or-above divergence; the protocol provides no policy state for those bindings to resolve against). See [../design/primitives/data/event-logs/iel/event-log.md §Effect on Bound SELs](../design/primitives/data/event-logs/iel/event-log.md#effect-on-bound-sels).
