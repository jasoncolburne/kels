...
- **Operator path between the two outcomes:** the operator's choice of response determines which path the chain takes. `Rec` resolves cleanly via archival; `Rot_op` racing the adversary's `Rot_adv` produces federation-level non-convergence. Operator-side monitoring catches adversary `Rot` before honest concurrent rotation, preventing the race; operational serialization on `Rot` issuance closes the race window. The contested-prefix table at the infrastructure layer ([#205](https://github.com/jasoncolburne/kels/issues/205)) surfaces the federation-level disagreement; operator response is reincept under a new prefix when reconciliation fails.
- **Detection:** Divergence detection as above. The federation-layer contested-prefix table surfaces cross-node disagreement when priv-vs-priv races occur.
...
- **Mitigation:** The merge layer rejects any such submission per [../design/protocol-doctrine.md §Privileged Divergence is Terminal](../design/protocol-doctrine.md#privileged-divergence-is-terminal). The chain stays at its prior state. Cross-node priv-vs-priv races surface at the federation layer via the contested-prefix table — operator recourse is out-of-band reconciliation or reincept.
...
- **Mitigation:** Each `Evl` lands cleanly as a linear-chain extension on its submitting node and advances the local seal. Gossip then delivers each event to the other node, where the seal-cap rejects the late arrival. Per-node, each chain stays linear; cross-node, the contested-prefix table at the federation layer surfaces the disagreement (every IEL event is privileged, so a second event at the same serial is uniformly rejected at merge per [../design/protocol-doctrine.md §Privileged Divergence is Terminal](../design/protocol-doctrine.md#privileged-divergence-is-terminal)). No protocol-level distinction between accidental race and threshold compromise; consumer/application response is the same (reincept under a new IEL prefix when reconciliation fails).
...
- **Mitigation (structural — mutual destruction):** When the legitimate operator submits their own `[Icp, Est_op]`:
  ...
  - Cross-federation: each node sees one of `Est_camper` / `Est_op` landing first; gossip-arriving competing Est is rejected by the seal-cap on each node. Federation-level disagreement on the prefix is surfaced via the contested-prefix table at the infrastructure layer (see [../design/protocol-doctrine.md §Limit of the doctrine — concurrent privileged event races](../design/protocol-doctrine.md#concurrent-privileged-event-races) and [#205](https://github.com/jasoncolburne/kels/issues/205)).
...
- **Residual visibility cost:** The federation-disputed prefix is surfaced via the contested-prefix table as forensic record; the chain cannot extend forward federation-wide — no consumers honor authorizations rooted in a disputed prefix.
...
KELS primitives chain authority: KELs anchor IEL governance acts; IELs root SEL authorization. When one chain is disrupted at the federation layer (via the contested-prefix table), dependent chains whose bindings reach at-or-below-seal chain state stay authorized; only forward extensions that would bind against above-seal state on the disrupted chain face the freeze. See [../design/protocol-doctrine.md §Pre-seal verifiability](../design/protocol-doctrine.md#pre-seal-verifiability).

### KEL Federation-Dispute Cascade

**Scenario:** A KEL is disputed at the federation layer (e.g., a tier-2 adversary races `Rot_adv` against an honest `Rot_op` on different nodes; the contested-prefix table surfaces the disagreement).
...
**Scenario:** An IEL is disputed at the federation layer (every IEL event is privileged, so divergent sets cannot form locally; cross-node priv-vs-priv races surface via the contested-prefix table).
