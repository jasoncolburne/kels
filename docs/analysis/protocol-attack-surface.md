...
- **Recovery via `Rec`:** Owner submits `rec` with `previous = v_{N-1}.said` (divergence-ancestor-extending shape), dual-signed against `v_{N-1}`'s commitments — signing key revealed by the adversary's `rot` (both parties have it) AND recovery key (only the operator has it; recovery is revealed only by `rec`/`ror`/`dec`). Operator's dual-sig succeeds; adversary's does not. `Rec` lands at `v_N`; the discriminator archives the adversary's branch; the chain recovers, linear, tip = `Rec` at `v_N`. See [../design/primitives/data/event-logs/kel/event-log.md §Recourse against signing-key-only Rot takeover](../design/primitives/data/event-logs/kel/event-log.md#recourse-against-signing-key-only-rot-takeover).
- **Federation-non-convergence path via `Rot` race:** A rotation-tier adversary can also force federation-level non-convergence by racing `Rot_adv` against a concurrent honest `Rot_op` on different federation nodes. Each `Rot` lands as a clean linear-chain extension on its submitting node and advances the local seal; gossip then delivers each event to the other node, where the seal-cap rejects the late arrival. Per-node, both chains stay linear; cross-node, the federation does not converge at the protocol layer. The bar to disrupt the KEL via this path is rotation-tier compromise — strictly easier than the full tier-3 compromise required to forge `Ror`/`Dec`. Events at-or-below each node's `lastSealAdvancingEvent` retain structural verifiability per [../design/protocol-doctrine.md §Pre-seal verifiability](../design/protocol-doctrine.md#pre-seal-verifiability), bounding the damage to forward extension. Tier-1 `Ixn`s the adversary may have planted between the last seal and the Rot race are NOT structurally trustworthy — they sit above the seal and are not consumer-durable until a (now-impossible) future seal would advance past them.
- **Operator path between the two outcomes:** the operator's choice of response determines which path the chain takes. `Rec` resolves cleanly via archival; `Rot_op` racing the adversary's `Rot_adv` produces federation-level non-convergence. Operator-side monitoring catches adversary `Rot` before honest concurrent rotation, preventing the race; operational serialization on `Rot` issuance closes the race window. The irreconcilable-prefix table at the infrastructure layer ([#205](https://github.com/jasoncolburne/kels/issues/205)) surfaces the federation-level disagreement; operator response is reincept under a new prefix when reconciliation fails.
...
The "Rotation key compromised" subsection above covers the tier-2 adversary in full. The cross-doc walkthrough of the clean Rec recovery path lives at [../design/primitives/data/event-logs/kel/event-log.md §Recourse against signing-key-only Rot takeover](../design/primitives/data/event-logs/kel/event-log.md#recourse-against-signing-key-only-rot-takeover). The Rot-race federation-non-convergence path is the doctrinal threat-model shift; see [../design/protocol-doctrine.md §Tier-2 adversary federation-non-convergence path](../design/protocol-doctrine.md#tier-2-adversary-federation-non-convergence-path).

**Recovery key compromised (along with rotation key):**
- **Attack:** Adversary has full administrative control — can submit `rec`, `ror`, or `dec` events.
- **Impact:** Total identity compromise. Both parties hold the same authorities.
- **Outcome:** No in-band recourse remains. Either party can submit any recovery-revealing event; the adversary's events satisfy the same dual-sig check as the operator's. Concurrent submissions to different federation nodes produce federation-level non-convergence: each event lands cleanly on its submitting node, gossip-arriving competing event rejected by the seal-cap, federation surfaces disagreement via the irreconcilable-prefix table. The chain mathematics cannot adjudicate which party is legitimate; forward extension is structurally blocked at the federation level.
- **Recourse:** Reincept the KEL prefix under a fresh key set. Dependent chains rebind to the new prefix (out-of-band coordination at the IEL governance layer for chains that anchor to this KEL: an `Evl` on the dependent IEL rotates the disputed KEL out of `governancePolicy`).
