...
- Error codes: `BadRequest`, `NotFound`, `Conflict`, `Frozen`, `Unauthorized`, `Gone`, `ParentLocked`, `RateLimited`, `InternalError`. Cross-node priv-vs-priv races surface via the federation-level contested-prefix table (see [../design/protocol-doctrine.md §Limit of the doctrine — concurrent privileged event races](../design/protocol-doctrine.md#concurrent-privileged-event-races)) rather than via a per-primitive error code.
