...
`SadEventVerification` is the proof-of-verification token. It can only be obtained through the verifier. Accessors: `current_record()`, `current_content()`, `prefix()`, `write_policy()`, `topic()`, `policy_satisfied()`, `last_governance_version()`.
