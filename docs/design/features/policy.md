...
A `threshold(N, id_1, ..., id_M)` policy with `M > N` tolerates loss of up to `M − N` identities' authority while remaining satisfiable. If an anchor was originally satisfied by some subset of `N` identities and one of those identities is later contested (per [kel/event-log.md](../primitives/data/event-logs/kel/event-log.md) / [iel/event-log.md](../primitives/data/event-logs/iel/event-log.md)), the original anchor loses authority — but a new anchor under the same policy can be created using a different subset that excludes the contested identity, satisfying the threshold again. The underlying SAID's authorization is re-established without changing the policy itself.
