...
A `threshold(N, id_1, ..., id_M)` policy with `M > N` tolerates loss of up to `M − N` identities' authority while remaining satisfiable. If an anchor was originally satisfied by some subset of `N` identities and one of those identities is later disrupted at the federation layer (e.g., surfaced as in-dispute via the irreconcilable-prefix table per [../protocol-doctrine.md §Limit of the doctrine — concurrent privileged event races](../protocol-doctrine.md#concurrent-privileged-event-races)), the original anchor loses authority — but a new anchor under the same policy can be created using a different subset that excludes the disrupted identity, satisfying the threshold again. The underlying SAID's authorization is re-established without changing the policy itself.
