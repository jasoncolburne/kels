...
- Server submit handler — terminal gate, immunity gate, divergent-rejection routing (returns `ContestRequired` for events on divergent chains, since IEL has no repair primitive and no Cnt), algorithmic `ContestRequired` trigger for events at-or-before evaluation seal. (Post-#202 code follow-up: remove the `is_contest` routing branch.)
