...
| `Sea` | `kels/sel/v1/events/sea` | Seal — governance evaluation. Advances `lastSealAdvancingEvent`. No policy-field evolution (policies live on IEL); may advance `ielEvent` to a newer IEL state. Back-to-back Sea allowed only when the new Sea **strictly advances** `ielEvent` — see [../../../../protocol-doctrine.md §Shape constraints on Sea](../../../../protocol-doctrine.md#shape-constraints-on-sea). |
...
- **`Est`-`Est` race at v=1 is mutually destructive.** `Est` is privileged at tier 2; an `Est`-`Est` divergent set fires privileged-divergence-is-terminal at first observation. Neither the camper nor the operator gets a working chain at the contested `(identity, topic)`. Operator recourse against a successful camp is reincept under a new `(identity, topic)` tuple.

The four rules compose: `Icp` permissionless preserves dedup-idempotency on the prefix-deterministic inception; `Est` tier-2 raises per-attempt camping cost; inception batch required closes the "lone `Icp` placeholder" gap; `Est`-`Est` mutual destruction means a successful camp dies along with the operator's attempt, removing any "camper holds the chain" outcome. Together they make SEL camping expensive **and** unprofitable: the camper pays tier-2 cost to deny the operator a tuple they can both abandon.
...
- **The bound IEL event is acceptable iff** the IEL is non-divergent AND `bound_event.serial <= bound_iel.lastSealAdvancingEvent.serial`, OR the IEL is divergent AND `bound_event.serial <= bound_iel.lastSealAdvancingEvent.serial` AND `bound_event.serial < first_divergent_serial`. Both the seal-bound and (for divergent IELs) the divergence-bound must hold. A binding above the IEL's `lastSealAdvancingEvent` is rejected with `IelDivergent` — events above the seal are not structurally trustworthy as authorization context per [../../../../protocol-doctrine.md §Pre-divergence verifiability survives contestation](../../../../protocol-doctrine.md#pre-divergence-verifiability-survives-contestation). A binding at-or-after `first_divergent_serial` on a divergent IEL is rejected for the same reason — the IEL has no single authoritative state at that point.

  > **Note: chain-validity ≠ consumer trust.** An at-or-below-seal binding to a contested IEL passes the verifier and stays trust-evaluable for consumer queries — at-or-below-seal IEL events retain structural verifiability per [../../../../protocol-doctrine.md §Pre-divergence verifiability survives contestation](../../../../protocol-doctrine.md#pre-divergence-verifiability-survives-contestation). The contested IEL is forward-terminal: a SEL that needs to advance its binding above the IEL's seal cannot, since above-seal IEL events are not structurally trustworthy (in the gap between seal and divergence) or are submitter-indistinguishable (at-or-above divergence); that's the case where the operator reincepts the SEL under a new IEL prefix (see [../iel/event-log.md §Effect on Bound SELs](../iel/event-log.md#effect-on-bound-sels)).
...
Two governance-authorized parties — both satisfying the chain's IEL-resolved `governancePolicy` at `v_{d-1}` — submit concurrent privileged events extending the same parent `v_{d-1}` to different nodes. Each event extends `v_{d-1}` and lands at `v_d` on its submitting node. Once gossip merges the two events, the divergent set at `v_d` forms, privileged-divergence-is-terminal fires, and the chain becomes contested-terminal.

```
v0..v3   normal linear chain
v4       Sea       previous=v_3.said, serial=4     ← party 1's Sea (on node A)
v4'      Dec       previous=v_3.said, serial=4     ← party 2's Dec (on node B)
    — 2-event divergent set at v_4 once both propagate via gossip: {Sea, Dec}.
