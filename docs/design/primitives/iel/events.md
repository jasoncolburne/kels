...
The two events at `v_2` carry the same `previous = v_1.said` — each was accepted as a linear-chain extension on its submitting node at submission time, with the two extensions independently landing at `v_2`. Valid 2-event pairings on IEL are `Evl`-`Evl`, `Evl`-`Cnt`, and `Evl`-`Dec` — every divergent set at `v_d` contains at least one `Evl`. `Cnt` and `Dec` are each absolute and terminal (at most one per log; the contested- or decommissioned-state gate locks after first acceptance), so `Cnt`-`Cnt`, `Dec`-`Dec`, and `Cnt`-`Dec` cannot form. The outcome is the same in every valid case: every IEL event is privileged, so the divergent set transitions the chain to contested-terminal immediately by the privileged-divergence-is-terminal rule. **No 3rd event lands at `v_2`** — the contested-state gate rejects all subsequent submissions, including any further `Evl`, `Cnt`, or `Dec` arriving at v_2 via gossip. **Once divergence is observed, no Cnt is accepted on a divergent IEL** — Cnt acceptance is always a linear-chain extension on the submitting node's local chain; the 2-event divergent set is emergent (observed via gossip-merge of two independently-submitted linear-chain extensions, or as the post-acceptance state on a node whose tip is the gossip-delivered concurrent event when Cnt lands — see next example).
