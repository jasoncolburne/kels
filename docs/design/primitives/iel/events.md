...
`Sea`'s `version >= 2` constraint and the parent-kind constraint (`Sea`'s parent must be `Evl` — see §Per-Kind Policy Field Discipline) are co-implied: only `Evl` can land at v=1 on a non-terminal IEL chain (Icp is at v=0; Sea/Cnt/Dec are forbidden at v=1 by version rules or terminal semantics), so the parent at `v_{tip-1}` of any `Sea` at v=2 is necessarily an `Evl`. The two constraints are stated independently for clarity but describe the same structural shape.
