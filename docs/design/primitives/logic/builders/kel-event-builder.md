...
Constructs structurally-valid KEL events (`Icp`, `Dip`, `Ixn`, `Rot`, `Ror`, `Rec`, `Dec`), wiring `previous`, `serial`, forward-key commitments (`rotationHash`, `recoveryHash`), and dual signatures on the recovery-revealing kinds. Used by callers that need to produce KEL events directly.
