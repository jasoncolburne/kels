...
**Distinction from KEL Rec / SEL Rpr.** KEL Rec and SEL Rpr resolve divergence by archiving events via the discriminator. They take two parent shapes: branch-tip-extending (`previous = v_d.said` for one of the two branch tips, lands at `v_{d+1}`, archives the other branch) and divergence-ancestor-extending (`previous = v_{d-1}.said`, lands at `v_d`, archives both branches at `v_d`). Cnt does NOT archive — it joins the existing divergent set as a 3rd event at `v_d` (parent = `v_{d-1}`), privileged-divergence-is-terminal fires, the chain transitions to contested-terminal. The kind discriminator determines whether the chain recovers (Rec/Rpr; archival) or terminates (Cnt; no archival). IEL has no Rec/Rpr equivalent — there is no recovery primitive, only termination.
...
`Cnt.previous = v_{tip-1}.said` — the parent of the chain's current tip on a linear chain (creates fresh divergence at `v_N`), or `v_{d-1}` on a divergent chain (the divergence ancestor). On IEL, both divergent branches are single-event at `v_d` by construction — every IEL event advances the seal, so the chain transitions to contested-terminal at first 2-event divergence and the pre-existing branch cannot extend past `v_d`. Each branch's `v_{tip-1}` is `v_{d-1}` — same `v_{tip-1}` rule applied to a different chain shape. On IEL specifically, the chain transitions to contested-terminal as soon as 2-event divergence is observed (every IEL event is privileged → privileged-divergence-is-terminal fires), so a Cnt arriving after that point is rejected by the contested-state gate. Cnt only lands as one of the 2 events of the original divergent set, or as the linear-chain operator-initiated termination. Cross-node propagation works because `v_{d-1}` is structurally shared (lands cleanly before any divergence) — Cnt with `previous = v_{d-1}.said` validates uniformly across nodes.
...
**Threshold redundancy** is the operator's per-anchor recovery option for partial compromise: an anchored policy with `M > N` identities tolerates loss of `M − N` while remaining satisfiable, and a new anchor using a different threshold-satisfying subset re-establishes authorization without changing the policy itself (see [../policy.md §Threshold Redundancy](../policy.md#threshold-redundancy)).
