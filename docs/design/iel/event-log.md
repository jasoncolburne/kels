# Identity Event Log (IEL) — Lifecycle, Repair, Contest, Decommission

> Source-of-truth design doc for the IEL chain lifecycle. Pairs with [reconciliation.md](reconciliation.md) (multi-node correctness proof matrix), [merge.md](merge.md) (submit-handler routing and `truncate_and_replace` discriminator), and [verification.md](verification.md) (IelVerifier algorithm).
...
|---|---|---|
| **Active** | Linear chain of events, latest tip extends cleanly. | Yes — `Sea`, `Rpr`, `Cnt`, `Dec` (per `governance_policy`). |
| **Divergent** | Two events exist at some version `d`. Chain is frozen until repaired. | Only `Rpr` (governance authorized). |
...
| `Sea` | Seal — governance evaluation; advances the seal, may evolve `auth_policy` and/or `governance_policy`. | `governance_policy`. | No |
| `Rpr` | Repair — advances the seal AND resolves divergence. | `governance_policy`. | No |
...
2. Read the policy declared (`Icp`) or evolved (`Sea`) at that event.
3. Verify SE.said is anchored under that policy.

For this resolution to remain deterministic forever:
- `IEL_event_X` must remain in IEL's authentic chain (never archived) — guaranteed by chain immutability and the seal property.
...
- That IEL event declared (`Icp`) or evolved (`Sea`) the relevant policy — `auth_policy` for SE `Upd`, `governance_policy` for SE `Sea`/`Rpr`/`Cnt`/`Dec`.
...
When the IEL's `governance_policy` evolves (a Sea on IEL changes who has governance authority), the operator should immediately submit a `Sea` on each dependent SE chain to ratchet that chain's `last_identity_event` forward to the new IEL Sea. This closes the window in which an adversary with revoked governance could submit a stale-bound `Cnt`/`Dec` against an unmaintained SE chain.
...
- An IEL `Sea` / `Rpr` / `Cnt` / `Dec` whose policy was satisfied entirely by owner-placed anchors: re-verifies cleanly across `rec`. Past evaluation stands. SE chains bound to that IEL event continue to verify under it.
...
Divergence is detected when two IEL events share the same `previous` SAID. The chain is frozen at that point: the merge engine rejects all non-repair appends with the existing "Chain is divergent — repair required" path.
...
The divergence invariant — combined with the proactive governance evaluation requirement (`MAX_NON_EVALUATION_EVENTS = MINIMUM_PAGE_SIZE - 1 = 63`, which is automatic on IEL since every event after Icp is governance-authorized) — guarantees:
- Exactly 2 events at the divergence version `d`.
- At most 1 event at each version `> d` (chain frozen post-divergence).
...
Mechanically identical to SEL repair (see [../sel/event-log.md §Repair](../sel/event-log.md#repair-rpr)). The discriminator algorithm, builder boundary derivation, pending-events bundling, and bounds all parallel SEL.

### Special concern: SE chains bound to a repaired-out IEL event

If the IEL repair archives an IEL `Sea` event that some SE chain had bound to via `identity_event`, those SE bindings now reference an IEL event that no longer exists in the authentic chain. SE verification of those bindings will fail (the binding doesn't resolve to an authentic IEL event).

This is the right behavior: an IEL Sea on the adversary branch was an unauthorized policy evolution. SE events bound to it were authorized under the adversary's policy, which is illegitimate. Repair correctly invalidates them.

The SE chain's recovery path: if the adversary's IEL-side actions affected the SE chain's authorization context, the SE chain may itself need an `Rpr` (if divergent) or `Cnt` (if the legitimate operator cannot defeat the adversary going forward). The cross-chain consequences propagate through standard SE lifecycle mechanisms.

## Contest (Cnt)

Contest is the terminal state for authority conflict — the legitimate operator cannot defeat an adversary who has demonstrated `governance_policy` authority on the IEL. `Cnt` freezes the IEL.

### Server semantics

- Verify `Cnt`'s structure, governance authorization.
- Insert `Cnt`. **No archival** — both branches preserved for forensic review.
...
- Pre-flight: full chain re-verification (`verify_server_chain_pre_action`).
- Bundles pending events into the batch (mirrors SEL).
- Builds `Cnt` extending the last bundled event.
...
Decommission is the clean terminal state for owner-initiated identity end. Same shape as SEL `Dec`.
...
|---|---|---|
| Linear, normal append | `Sea` | Append. Seal advances. |
...
| Linear, post-evaluation-seal | `Sea` extending pre-seal version with valid governance | `ContestRequired { reason }` (mirrors SEL). |
| Any | `Cnt` | Insert, mark contested. |
| Any | `Dec` | Insert, mark decommissioned. |
...
**Code:**
- `lib/kels/src/types/iel/event.rs` — `IdentityEventKind` enum (`Icp`/`Sea`/`Rpr`/`Cnt`/`Dec`); `validate_structure` per per-kind field rules.
...
- `lib/kels/src/identity_builder.rs` — `IdentityEventBuilder` with `seal()`, `repair()`, `contest()`, `decommission()`; pending-events bundling; pre-flight server-chain re-verification.
- Server (`services/sadstore/src/handlers.rs` or new `services/idstore/`) — submit handler with terminal gate, immunity gate, divergent-rejection routing, `ContestRequired` algorithmic trigger.
- Storage — `iel_events` table (parallel to `sad_events`); `iel_event_archives` for repair archival.

**Tests:**
- Mirrors SEL's test surface: submit / verifier / builder / discriminator / page-tamper.
