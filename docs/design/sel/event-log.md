...
**Once an evaluation lands, the governance satisfaction it proves is final.** More broadly, **once any past write or governance event lands, its authorization is final.** This is enforced *structurally* via a constraint on policies introduced or evolved on the chain:

> **Policy immunity rule.** Any policy referenced as a chain's `write_policy` or `governance_policy` MUST have `immune: true`. Both the merge engine (at submit time) and the verifier (at verification time) reject any `Icp`, `Est`, or `Sea` event that introduces or evolves a policy whose `immune` flag is not set. Both layers enforce because the verifier processes data from any source — gossip, peer pulls, restored backups — and cannot trust that the originating node enforced the rule (the "DB cannot be trusted" invariant; see [../security-invariant.md](../security-invariant.md)).

Since `immune: true` makes a policy impervious to poisoning in the evaluator (`evaluate_anchored_policy` skips poison checks for immune policies), the rule guarantees that no anchor used in any chain authorization (write or governance) can ever be poisoned. Past `Sea`/`Rpr`/`Cnt`/`Dec` evaluations and past `Upd` authorizations stay satisfied by construction — there is no mechanism by which they could be retroactively unsatisfied.

The rule is the "bake state into data" principle applied to an authorization invariant: any authorized observer can determine from a policy SAID alone whether the stability property holds for chains that reference it. Immunity is structural, not behavioral; it does not depend on evaluator carve-outs or runtime semantics.

**Revocation via policy evolution, not poison.** To remove an endorser's authority going forward, evolve the policy via `Sea` (declaring a new `write_policy` or `governance_policy` SAID that excludes the endorser); the new policy must itself be immune. Past events stay authorized under the policy that was in effect when they landed. For compromise of an underlying anchoring KEL, the corrective mechanism is `rec` / `cnt` on that KEL (see [§Trust Caveat below](#trust-caveat--recovered-anchoring-kels)).
