...
**Recourse against Tier-2 Rot takeover (KEL specifically)**: an adversary holding the rotation-key preimage at `v_N` (revealing their `Rot` at `v_N`) does not hold the recovery-key preimage committed by the prior establishment's `recoveryHash`. A `Rec` (branch-tip-extending on a divergent chain, or divergence-ancestor-extending where the divergence ancestor's commitments are still legitimate) — subject to the locked-portion bound (condition 2b) — resolves dual-sig against the parent's commitments; the legitimate party's recovery-key preimage satisfies, the adversary's does not. See [primitives/data/event-logs/kel/event-log.md §Recourse against Tier-2 Rot takeover](primitives/data/event-logs/kel/event-log.md#recourse-against-tier-2-rot-takeover) for the key-state walkthrough.
...
Cross-node disagreement on which privileged event won a federation race is a federation-level concern, surfaced via the irreconcilable-prefix table at the infrastructure layer (see [§Limit of the doctrine — concurrent privileged event races](#concurrent-privileged-event-races)). It is not a per-node chain state.
...
**The federation's convergence layer for these races is the irreconcilable-prefix table** — see [#205](https://github.com/jasoncolburne/kels/issues/205) for the design. Nodes maintain and gossip-sync signed attestations of per-node state for prefixes where peer views disagree; the table is the canonical "is this chain in dispute?" answer for consumers. Cross-node divergence — priv-vs-priv races, priv-vs-non-priv state mismatches, divergent vs Decommissioned — is captured uniformly there. The protocol layer enforces local invariants strictly; the federation layer surfaces all cross-node disagreement.
...
If an adversary acquires sufficient (privileged) currently-controlling authority — current KEL rotation or rotation+recovery pre-images; current IEL `governancePolicy` threshold; current SEL identity binding's authorizing IEL event — they ARE the chain's current state by every protocol-observable measure. They can:
...
Two compromise paths produce federation-level non-convergence the protocol layer alone cannot resolve. Both involve racing privileged events across federation nodes; both are bounded by [§Pre-seal verifiability](#pre-seal-verifiability) and surfaced via the irreconcilable-prefix table at the infrastructure layer ([#205](https://github.com/jasoncolburne/kels/issues/205)).

**Tier-2 path.** An adversary holding the rotation-key preimage — and only the rotation-key preimage — can forge `Rot` events (single-signed by the new signing key the preimage reveals; the old signing key is not a prerequisite). Racing `Rot_adv` against an honest concurrent `Rot_op`/`Ror_op` on different federation nodes produces cross-node non-convergence: each `Rot` lands as a clean linear-chain extension on its submitting node and advances the local seal; gossip delivers each to the other node where the seal-cap rejects the late arrival. The forging bar is rotation-tier compromise (one preimage), strictly easier than the full tier-3 compromise required to forge `Ror`/`Rec`/`Dec`.

**Tier-3 path.** An adversary holding both the rotation-key preimage AND the recovery-key preimage can forge any recovery-revealing event (`Ror`/`Rec`/`Dec`) — dual-signed by the new signing key the rotation preimage reveals and the recovery key the recovery preimage reveals (the old signing key is not a prerequisite). Racing those events against operator submissions on different federation nodes produces the same cross-node non-convergence shape: each event lands cleanly on its submitting node and advances the local seal; gossip-arriving competing event is rejected by the seal-cap. No in-band protocol recourse exists once an adversary's recovery-revealing event has landed on any node.

**Bounded damage.** Anchors, credentials, and SEL bindings at-or-below each node's `lastSealAdvancingEvent` retain structural verifiability regardless of which side of the race that node accepted, per [§Pre-seal verifiability](#pre-seal-verifiability). A tier-2 adversary extending with tier-1 `Ixn`s between the last seal and the Rot race, or a tier-3 adversary extending under fresh post-`Ror` key state, pollutes the above-seal range; those events are not structurally trustworthy under the seal-bound rule.

**Mitigations.** Operator monitoring catches adversary events before the honest concurrent submission; operational serialization on privileged-event issuance closes the race window; the irreconcilable-prefix table at the infrastructure layer ([#205](https://github.com/jasoncolburne/kels/issues/205)) surfaces the federation-level disagreement to consumers. The proposed in-band close for both tiers is federation witnessing ([#216](https://github.com/jasoncolburne/kels/issues/216)) — a federation-layer attestation requirement that converges per-node first-receive across the federation. See [../analysis/protocol-attack-surface.md §Key Compromise](../analysis/protocol-attack-surface.md#key-compromise-kel) for the worked threat scenarios.
...
Reincept is needed when forward extension on a chain is structurally blocked — typically when the federation has surfaced the chain as irreconcilable via the irreconcilable-prefix table (see [§Limit of the doctrine — concurrent privileged event races](#concurrent-privileged-event-races)) and the operator cannot proceed without a fresh prefix. Dependent chains whose bindings reach at-or-below-seal state on the disrupted chain stay authorized; only forward extensions that would bind against above-seal state face the freeze.
...
KELS depends on **eventual cross-node convergence**: gossip propagation, paired with deterministic effective-SAID computation, ensures every chain resolves to the same semantic state on every node in a healthy federation when convergence is possible at the protocol layer. Where the protocol layer cannot converge — concurrent privileged event races between nodes — the irreconcilable-prefix table at the infrastructure layer is the canonical convergence mechanism (see [§Limit of the doctrine — concurrent privileged event races](#concurrent-privileged-event-races) and [#205](https://github.com/jasoncolburne/kels/issues/205)).
...
- **Single-node-compromise mitigation** — depends on cross-node replication surfacing tampering as divergence (at the protocol layer) or as irreconcilable-prefix-table disagreement (at the infrastructure layer).
...
- **Federation-level disagreement surfaces uniformly.** If two governance-authorized `Evl`s land concurrently on different nodes, each event lands as a clean linear extension on its submitting node and the seal-cap rejects the gossip-arriving competing event. Per-node, each chain is linear; cross-node, the irreconcilable-prefix table surfaces the disagreement (see [§Limit of the doctrine — concurrent privileged event races](#concurrent-privileged-event-races)). The federation cannot extend the IEL further under either branch without operator-level reconciliation; the operational response is a fresh federation IEL inception under a new prefix. The disagreement signal converges at the infrastructure layer; convergence is not contingent on the chain's success.
...
- A privileged event extending `v_{N-1}` is rejected at the merge layer per [§Privileged Divergence is Terminal](#privileged-divergence-is-terminal) — it would create a divergent set containing a privileged event. The federation surfaces the conflict via the irreconcilable-prefix table.
