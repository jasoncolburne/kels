...
3. The exclusion `Evl` is **immediately batched with a `Sea`** to close the structural window the exclusion opens. Without a trailing `Sea`, the chain's highest-serial event is the exclusion `Evl` at `v_N`; any non-archiving privileged event (`Evl`/`Sea`/`Dec`) constructed against this chain with `previous = v_{N-1}.said` and `serial = N` lands at `v_N` alongside the exclusion `Evl` (per the parent-at-seal boundary case — see [protocol-doctrine.md §Privileged Divergence is Terminal §Repair-event conditions](../protocol-doctrine.md#privileged-divergence-is-terminal)). `v_{N-1}` predates the exclusion, so its `governancePolicy` is `P_old`, which any rotated-out party can still satisfy. Their non-archiving privileged event at `v_N` is structurally valid; privileged-divergence-is-terminal fires and the federation IEL becomes contested-terminal. The trailing `Sea` extends the chain to `v_{N+1}`: any subsequent privileged event has `previous = v_N.said`, whose `governancePolicy` is `P_new` (declared by the exclusion `Evl`), which no rotated-out party can satisfy by definition. The `Sea` itself is authorized under `P_new`, so the operator can land it but the rotated-out party cannot. This concern is specific to **exclusion** evolutions (member removed or replaced); pure additions or threshold-decreases that keep prior membership open up no rotated-out position and do not need the trailing `Sea`. See [protocol-doctrine.md §Exclusion Evolutions and the Seal Advance](../protocol-doctrine.md#exclusion-evolutions-and-the-seal-advance). The federation-membership CLI should bake `[Evl, Sea]` into every exclusion evolution.
