...
- **Detect federation-dispute outcomes per topic.** If a `(identity, topic)` is surfaced as in-dispute via the contested-prefix table (race lost on at least one federation node), the enrollment retries under a different `(identity, topic)` — typically a different `identity` (fresh IEL prefix) since `topic` is application-determined.
