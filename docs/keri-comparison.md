...
KELS derives the prefix differently from the SAID (blanking both `said` and `prefix` fields before hashing), decoupling identity from inception content.
...
KELS's prefix derivation offers a subtle privacy advantage: because the prefix is computed differently from the SAID (blanking both fields vs only `said`), a naked SAID seen in an anchor or external reference cannot be trivially mapped to a KEL prefix without additional context. In KERI, prefix = inception SAID, so any reference to the inception SAID immediately identifies the KEL.
