...
`cmd_exchange_publish_key` and `cmd_exchange_rotate_key` returned errors with descriptive "parked pending Gap 11 CLI rewrite" messages. The pre-round-12 flow declared `write_policy` / `governance_policy` inline at inception and computed the SEL prefix from `(write_policy, topic)`. Round-12 SELs bind to an existing IEL via `identity` instead. Gap 11 reshapes the CLI surface to feed an IEL identity into `incept_chain(identity, topic, initial_content)`.
