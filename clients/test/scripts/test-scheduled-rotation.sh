...
ROTATION_KINDS=$(echo "$ALL_EVENTS" | jq -r '[.[] | .event | select(.kind == "kels/kel/v1/events/rot" or .kind == "kels/kel/v1/events/ror") | .kind] | .[]')
...
run_test "3rd rotation is ROR" [ "${KINDS[2]}" = "kels/kel/v1/events/ror" ]

# Verify 4th rotation (index 3) is ROT
run_test "4th rotation is ROT" [ "${KINDS[3]}" = "kels/kel/v1/events/rot" ]

# Verify 1st rotation (index 0) is ROT
run_test "1st rotation is ROT" [ "${KINDS[0]}" = "kels/kel/v1/events/rot" ]

# Verify 2nd rotation (index 1) is ROT
run_test "2nd rotation is ROT" [ "${KINDS[1]}" = "kels/kel/v1/events/rot" ]
