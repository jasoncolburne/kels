...
# Rotate on node-a
run_test "Rotate signing key on node-a" kels-cli --kels-url "$NODE_A_URL" kel rotate --prefix "$PREFIX1"
...
run_test "Anchor data on node-a" kels-cli --kels-url "$NODE_A_URL" kel anchor --prefix "$PREFIX1" --said "$TEST_SAID"
...
PREFIX4=$(kels-cli --kels-url "$NODE_A_URL" kel incept 2>&1 | grep "Prefix:" | awk '{print $2}')
echo "Created KEL on node-a: $PREFIX4"

# Submit multiple anchors rapidly
kels-cli --kels-url "$NODE_A_URL" kel anchor --prefix "$PREFIX4" --said "KGossipMulti1_______________________________"
kels-cli --kels-url "$NODE_A_URL" kel anchor --prefix "$PREFIX4" --said "KGossipMulti2_______________________________"
kels-cli --kels-url "$NODE_A_URL" kel anchor --prefix "$PREFIX4" --said "KGossipMulti3_______________________________"
kels-cli --kels-url "$NODE_A_URL" kel rotate --prefix "$PREFIX4"
kels-cli --kels-url "$NODE_A_URL" kel anchor --prefix "$PREFIX4" --said "KGossipMulti4_______________________________"
...
kels-cli --kels-url "$NODE_B_URL" kel get "$PREFIX4" > /dev/null 2>&1 || true
...
if kels-cli --help 2>&1 | grep -q "adversary"; then
    # Create new KEL
    PREFIX6=$(kels-cli --kels-url "$NODE_A_URL" kel incept 2>&1 | grep "Prefix:" | awk '{print $2}')
    echo "Created KEL on node-a: $PREFIX6"

    kels-cli --kels-url "$NODE_A_URL" kel anchor --prefix "$PREFIX6" --said "KPreDivergence______________________________"
    ...
    kels-cli --kels-url "$NODE_A_URL" kel anchor --prefix "$PREFIX6" --said "KOwnerCausesDivergence______________________" 2>&1 || true
...
if kels-cli --help 2>&1 | grep -q "adversary"; then
    ...
    if [ -n "$PREFIX6" ]; then
        # Recover on node-a
        run_test "Owner recovers on node-a" kels-cli --kels-url "$NODE_A_URL" kel recover --prefix "$PREFIX6"
...
PREFIX8=$(kels-cli --kels-url "$NODE_A_URL" kel incept 2>&1 | grep "Prefix:" | awk '{print $2}')
...
run_test "Decommission KEL on node-a" kels-cli --kels-url "$NODE_A_URL" kel decommission --prefix "$PREFIX8"
...
run_test "Node-b shows dec event" [ "$LAST_KIND" = "kels/kel/v1/events/dec" ]
