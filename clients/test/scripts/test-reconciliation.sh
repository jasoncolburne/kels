...
all_nodes_contested() {
    ...
    for url in "$NODE_D_URL" "$NODE_E_URL" "$NODE_F_URL"; do
        ...
        local has_cnt
        has_cnt=$(echo "$events" | jq '[.[].event.kind] | any(. == "kels/kel/v1/events/cnt")')
...
save_adversary_keys
swap_to_adversary
kels-cli --kels-url "$NODE_D_URL" adversary inject --prefix "$PREFIX6" --events rec
...
swap_to_adversary
run_test_expect_fail "Second rec rejected" kels-cli --kels-url "$NODE_D_URL" adversary inject --prefix "$PREFIX7" --events rec
swap_to_owner
