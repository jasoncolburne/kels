...
> Identity-rooted under [IEL](#iel-identity-event-log) — each SEL binds at inception to a specific IEL via `identity`; every v1+ event resolves its authorization via the IEL event referenced by `identity_event`.
